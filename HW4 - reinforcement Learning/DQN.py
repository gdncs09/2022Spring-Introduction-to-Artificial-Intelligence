...
class Agent():
    ...
    def learn(self, res):
        ...
        # Begin your code
        states_batch, actions_batch, rewards_batch, next_states_batch, done_batch=self.buffer.sample(self.batch_size)
        states = torch.FloatTensor(np.array(states_batch))
        actions = torch.LongTensor(np.vstack(actions_batch))
        rewards = torch.FloatTensor(np.vstack(rewards_batch))
        
        next_states = torch.FloatTensor(np.array(next_states_batch))
        done = torch.LongTensor(np.vstack(done_batch))    
        
        q_eval = self.evaluate_net.forward(states).gather(1, actions)
        q_next = torch.max(self.target_net.forward(next_states).detach(),1,True)[0]
        q_tar = rewards + (1-done)*self.gamma * q_next
        #q_next = self.target_net.forward(next_states).detach()
        #q_tar = rewards + (1-done)*self.gamma * q_next.max(1)[0].view(self.batch_size,1)
        loss = F.mse_loss(q_eval, q_tar)
    ...
    def check_max_Q(self):
        ...
        '''index = np.unravel_index(np.argmax(self.qtable[state]), self.qtable[state].shape)
        max_q = self.qtable[state][index]'''
        #return max_q
        pass
        # End your code
