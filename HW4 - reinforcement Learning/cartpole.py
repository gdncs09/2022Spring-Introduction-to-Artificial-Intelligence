...
class Agent():
    ...
    def discretize_observation(self, observation):
        ...
        for i, value in enumerate(observation):
            s = self.discretize_value(value, self.bins[i])
            state.append(s)
        return state
        pass
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done, ep, res):
        """
        ...
        #if not done:
            #self.qtable[tuple(state)][action] += self.learning_rate*(reward + self.gamma*np.max(self.qtable[tuple(next_state)]) - self.qtable[tuple(state)][action])  
        #else:
        ...
        global best_reward  
        if (ep == episode-1) and (res > best_reward):
            best_reward = res
    ...
    def check_max_Q(self):
        ...
        #max_q = np.argmax(self.qtable[tuple(state),:])
...
def train(env):
    ...
    for ep in tqdm(range(episode)):
        ...
        while True:
            ...
            training_agent.learn(state, action, reward, next_state, done, ep, count)
